...
class NucliDataset(utils.Dataset):
  ...
  def load_image(self, image_id, image_path=None, gamma=2.0): #, img_h, img_w, img_ch, resize_mode='constant'):
        ...
        """
        if image_path == None:
          image_path = os.path.join( self.image_info[image_id]["path"],  'images/', str(self.image_info[image_id]["id"])+'.png')
        # Load image
...
def get_purple_ids():
    ...
    data = pd.read_csv('/content/drive/Kaggle/classes.csv')
    purple = data[data['foreground']=='purple']
    ids = [purple.iloc[i]['filename'].split('.',1)[0] for i in np.arange(len(purple))]
...
def train(model):
    ...
    if not(args.type is None) and (args.type == 'purple'):
        image_ids = get_purple_ids()
    elif not(args.type is None) and (args.type == 'all'):
        image_ids = os.listdir(dataset_dir)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect nucli.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/nucli/dataset/",
                        help='Directory of the Nucli dataset')
    ...
    parser.add_argument('--epochs', required=False, type=float, default=30,
                        metavar="the number of epochs",
                        help='the number of training epochs (default is 30)')
    ...
    parser.add_argument('--type', required=False, default='all',
                        metavar = "Images to use in training (all or just purple)",
                        help="Whether to train on all images ('all') or only purple image ('purple') (default is all)")
